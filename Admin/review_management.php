...
$stmt = $conn->prepare("SELECT review_id, user_id ,comment, rating FROM review");
$stmt->execute();
    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="adminNavbar">
            <ul class="nav navbar-nav">
                ...
                <li class="active"><a href="review_management.php">Manage Reviews</a></li>
            </ul>
...
<div class="container">
    <h2 class="text-center">Manage reviews</h2>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                <?php foreach ($admins as $admin): ?>
                    <tr>
                        <td><?= htmlspecialchars($admin['review_id']) ?></td>
                        <td><?= htmlspecialchars($admin['user_id']) ?></td>
                        <td><?= htmlspecialchars($admin['comment']) ?></td>
                        <td><?= htmlspecialchars($admin['rating']) ?></td>

                        <td class="action-icons">
                                <i class="glyphicon glyphicon-edit" title="Update" onclick="window.location.href='updatereview.php?id=<?= $admin['review_id'] ?>'"></i>
                                <i class="glyphicon glyphicon-trash" title="Delete" onclick="window.location.href='deletereview.php?id=<?= $admin['review_id'] ?>'"></i>
                            </td>
